...
export const OneArticle = ({ article, feed }: { article; feed: Feed }) => {
  ...
  const [createSavedArticleMutation, { loading: createSavedArticleLoading }] =
    useMutation(CREATE_SAVED_ARTICLE_MUTATION);
  const [deleteSavedArticleMutation, { loading: deleteSavedArticleLoading }] =
    ...
    <div className="grid grid-cols-12 rounded-lg py-4 px-4 border-4 border-gray-300">
      ...
      <div className="col-span-1 flex items-center justify-end">
        <a target="_blank" href={article.link}>
          <SingleArrorRight className="h-8 w-8 text-blue-500 inline-block align-middle" />
        </a>
      </div>
    </div>
  );
